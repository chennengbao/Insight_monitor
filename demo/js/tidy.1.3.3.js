 ...
 * @Last Modified time: 2017-11-22 17:40:08
 *
 * @config 配置信息
 * @deviceInfo 设备信息
...
!function() {
     ...
     * @return {[type]}     [description]
     */
    Domob.prototype.getXPath = function(ele) {
      // 判断是否是标签节点，如果是继续遍历父级节点
      for(var segs = []; ele && ele.nodeType == 1; ele = ele.parentNode) {
          ...
          if (!!ele['id']) {
              ...
              for (var n = 0; n < allNodes.length; n++) {
                  if (!!allNodes[n]['id'] && allNodes[n]['id'] == ele.id) {
                      uniqueIdCount++;
                  };       
                  if (uniqueIdCount > 1) {
                      break;
              ...
              // 遍历前一个兄弟标签
              for (var i = 0, sib = ele.previousElementSibling; sib; sib = sib.previousElementSibling) {
              ...
              if (i == 0) {
                  ...
                  if (ele.nextElementSibling) {
                      for(var j = 0, nsib = ele.nextElementSibling; nsib; nsib = nsib.nextElementSibling){
                          if (nsib.nodeName == ele.nodeName) {
                              j++;
                              // 找到就行，就可以确认它的序号为1
                              break;
                          }
                      ...
                      // 遍历后一个兄弟标签，无同类型兄弟标签
                      if (j == 0) {
                          segs.unshift(ele.nodeName.toLowerCase());
                      } else {
                          segs.unshift(ele.nodeName.toLowerCase() + '[' + (i + 1 )+ ']');
                      }
    ...
    /**
     * [handlerXpath 处理设置的xpath]
     * @return {[type]} [description]
     */
    Domob.prototype.handlerXpath = function() {
      for (var prop in _setting_.monitorXpath) {
        _setting_.regXpath[prop] = _setting_.monitorXpath[prop].toString().replace(/\*(?=(\/))/, '?.*').replace(/(?=(\/))|(?=(\*\[))|(?=(\@))|(?=(\'))|(?=(\"))|(?=(\[))|(?=(\]))/gi, '\\');
      };
    }
    ...
    Domob.prototype.getEleInfo = function(eleObj) {
        ...
        dataStorage.nodeXpath       =  '';
        dataStorage.regXpath        = '';
        dataStorage.regXpathAlias   = '';
        dataStorage.debug           = _setting_.debug;

        for (var prop in _setting_.regXpath) {
          if (new RegExp(_setting_.regXpath[prop], 'gi').test(this.getXPath(eleObj))) {
            dataStorage.nodeXpath = this.getXPath(eleObj);
            dataStorage.regXpath  = prop;
            dataStorage.regXpathAlias = _setting_.monitorXpath[prop];
            break;
          };
        };

        // dataStorage.nodeXpath = encodeURIComponent(this.getXPath(eleObj));
        dataStorage.nodeXpath = this.getXPath(eleObj);
    ...
    Domob.prototype.createUUID = function() {
        ...
        var _getIntAligner = function(radix) {
        ...
        var _hexAligner = _getIntAligner(16);

        return generate();
    }

    /*
     * [createUvPvId 生成UvId和PvId]
     * @return {[type]} [description]
     */
    Domob.prototype.createUvPvId = function() {
